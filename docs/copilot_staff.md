...
| --- | --- | --- | --- | --- | --- | --- |
| Copilot CLI（GPT-5.4） | `copilot --resume=fdc64aaa-c115-4144-99c3-7771e98aa2af` | 项目结构、构建入口、沙盒、地面敌人、飞行敌人原型、参数文档 | 进行中 | 已完成地面敌人正式模块化并接入主循环，补上击杀 +10 HKD 与玩家头顶 `+10` 飘字 | Combat reward 中间层重构等待 PlayerSandbox 侧改动稳定 | 下一位接手敌人正式战斗结算或飞行敌人模块化的人 |
...
| Copilot CLI（GPT-5.4） | `copilot --resume=afb13241-a3b1-49f4-8863-626279fc7f6e` | 玩家沙盒、敌人沙盒、MapDrawer 渲染、参数同步、主线 Player 接入 | 已完成 | 已完成玩家攻击/法术/HUD 接入主线 `Player.cpp`，并同步更新主程序入口、魂球 HUD 与技能表现 | 无 | 下一个负责玩家状态机、击退或更完整主线战斗的人 |
| Copilot CLI（GPT-5.4） | ` copilot --resume=ef54648a-46f6-4977-b24c-6dd9837bb4bb` | 参数整理、调参文档、员工文档、进度盘点、地图仓库 | 已完成 | 已完成基础参数归档、员工登记表建立与 resume 对应、地图仓库落地，并把 WorldSandbox 接成带老人/医生/房屋/商人的可漫游村庄 | 无 | 明天继续接手文档整理、参数同步或地图系统接线的 Copilot |
...
- 飞行敌人从当前 EnemySandbox 临时逻辑升级为正式敌人模块
...
- 当前飞行敌人还是沙盒内临时实现，尚未像地面敌人那样完全模块化
...
- 飞行敌人当前参数包括：移动冷却 8 帧、攻击冷却 70 帧、前摇 18 帧、火球步进 3 帧、索敌范围 12、火球伤害 1
- 当前击杀地面敌人后，玩家头顶会短暂显示 `+10` 飘字

**交接说明**

- 如果下一位要继续做敌人，优先把飞行敌人从 `src/sandbox/EnemySandbox.cpp` 抽成正式模块，并统一到 `include/enemy/Enemy.h` / `src/enemy/Enemy.cpp`
- 如果下一位要先做玩家战斗，就优先把玩家攻击、受伤、无敌帧与 EnemySandbox / 敌人模块打通，并把临时写在 Player 里的击杀奖励逻辑迁到中间结算层
